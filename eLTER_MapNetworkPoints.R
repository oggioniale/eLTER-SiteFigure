...
mapNetworkPoint <- function(networkID, countryCode) {
  lterNetworkSitesCoords <- jsonlite::fromJSON(networkID)
...
# example
mapNetworkPoint(networkID = 'https://deims.org/api/sites?network=7fef6b73-e5cb-4cd2-b438-ed32eb1504b3',
                countryCode = 'ITA')



### function getNetworkSites
getNetworkSites <- function(networkID) {
  lterNetworkSitesCoords <- jsonlite::fromJSON(networkID)
  lterSitesNetworkPointDEIMS <- sf::as_Spatial(sf::st_as_sf(lterNetworkSitesCoords, wkt = 'coordinates'))
  lterSitesNetworkPointDEIMS
}
# example
getNetworkSites(networkID = 'https://deims.org/api/sites?network=7fef6b73-e5cb-4cd2-b438-ed32eb1504b3')
...
fProduceMapOfSiteFromDEIMS <- function(deimsid, countryCode, listOfSites, gridNx, gridNy, width, height, siteName, bboxXMin, bboxXMax, bboxYMin, bboxYMax) {
  ...
  if (is.null(geoBoundaries)) {
    ...
    # tmap::tmap_save(
    #   tm = mapOfSite,
    #   filename = paste0("images/", paste0(gsub(" ", "_", siteName)), "_test.png"),
    #   dpi = 400,
    ...
    tmap::tmap_save(
      tm = mapOfSite,
      filename = paste0("images/", paste0(gsub(" ", "_", siteName)), ".png"),
      dpi = 400,
      insets_tm = mapOfCentroids,
      insets_vp = grid::viewport(gridNx, gridNy, width = 0.25, height = 0.25)
    )
...
##
# for produce all figure for LTER-Italy
##
...
centroidsFeatureSP@data
merged <- (dplyr::full_join(centroidsFeatureSP@data, listOfSites, by = 'SiteCode'))[-3,]

for (i in 1:nrow(merged)) {
  i = 62
  merged$SiteName[i]
  fProduceMapOfSiteFromDEIMS(
    deimsid = merged$DEIMS_ID[i],
    countryCode = 'ITA',
    listOfSites = sitesNetwork,
    gridNx = merged$gridNx[i] - .1,
    gridNy = merged$gridNy[i] + 0.05,
    width = 0.25,
    height = 0.25,
    siteName = merged$SiteName[i],
    bboxXMin = merged$bboxXMin[i],
    bboxXMax = merged$bboxXMax[i],
    bboxYMin = merged$bboxYMin[i],
    bboxYMax = merged$bboxYMax[i]
  )
}
